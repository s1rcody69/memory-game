...
const saveSession = (k)  => localStorage.setItem(SESSION_KEY, JSON.stringify(k));
...
export const register = (username, password) => {
  ...
  users[key] = {
    ...
    gamesPlayed: 0, bestMoves: null,
    streak: 0, lastWinDate: null,
...
export const login = (username, password) => {
  ...
  if (users[key].passwordHash !== simpleHash(password)) return { error: "Incorrect password." };
  saveSession(key);
  return { user: users[key] };
};
...
export const updateUserStats = (username, { moves }) => {
  ...
  if      (lastWin === today)     { /* same day — no change */ }
  else if (lastWin === yesterday) { streak += 1; }
  else                            { streak  = 1; }
  users[key] = {
    ...user,
